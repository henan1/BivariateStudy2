...
void myDraw::drawProgress(){
        ...
        char str[20];
        if(state.taskID == 0)
             sprintf(str,"Task 1: %d/108",order->dataPointer+1);
        else if(state.taskID == 1)
             sprintf(str,"Task 2: %d/108",order->dataPointer+1);
        else if(state.taskID == 2)
             sprintf(str,"Task 3: %d/150",order->dataPointer+1);
        else if(state.taskID == 3)
     ...
     if(order->pagePointer == order->pageNum){
        ...
        switch(order->pageOrder[order->pagePointer]){
           ...
           case 3:{
                     ...
                     && order->pageOrder[order->pagePointer-1]==1){
                           order->pagePointer--;
                     }
                     break;
                  }
...
void init(int argc, char **argv){
  ...
  correctFile = "file/correct" + string(numstr);
  logFile = "file/log" + string(numstr);
  ofstream log(logFile.c_str(),fstream::app);
  log<<order->datainfo->pID<<endl;
  log.close();
  answerFile = "file/result" + string(numstr);
  timeFile = "file/time" + string(numstr);
  confidenceFile = "file/confidence" + string(numstr);

  string blockFile = "order/orderWithInsert"+ string(numstr);
  order->readOrder(blockFile);
  string pageFile("order/pageOrder");
